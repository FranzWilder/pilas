...
<span class="badge bg-danger" id="msgError"></span>
    <div class="row">
        <div class="col-md-12 fondoAzul1 p-2">
            ...
            <div class="btn-group dropstart float-end">
                ...
                <ul class="dropdown-menu">
                    <li><a style="cursor:pointer" id="btnModalAsignar" class="dropdown-item"><i class="fas fa-plus-circle"></i> Asignar/ver usuarios</a></li>
         
                        <li><a style="cursor:pointer" id="btnGuardarReporte" class="dropdown-item"><i class="far fa-file-alt"></i> Generar Reporte</a></li>
               

                    <?php if ($usuario->cargo == "Supervisor"): ?>
                     
                        <li><a style="cursor:pointer" id="btnEliminarProyecto" class="dropdown-item"><i class="far fa-trash-alt"></i> Eliminar Proyecto</a></li>
                    <?php endif; ?>
    ...
    <div class="row  position-relative">
        <div class="col-md-12 position-relative divTotal" style="height: 600px;overflow: scroll;" >
            <div id="miMapa"></div>
        </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="fondoAzul1 text-white" style="padding: 26px;">
                <div class="row">

                    <div class="col-md-3" style="border-right:1px solid white">
                        <div>
                            <img class="float-start" src="static/images/pilaslogo.svg" style="height: 34px; padding-left: 8px;">
...
<div class="modal" tabindex="-1" id="modalAsignar">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="admin/proyectos/asignarUsuarios/<?php echo $idProyecto ?>" method="post">
                        ...
                        <tbody class="colorAzul4">
                            <?php foreach ($usuarios as $key => $value) : ?>
                                ...
                                <tr class="filaAsignado">
                                    <td style="background:#F7F7F7;"><img style="max-height:50px" class="img-fluid" onerror="if (this.src != '') this.src = 'static/images/perfil.png';" src="static/images/usuarios/<?php echo $value->id ?>/<?php echo $value->imagen ?>" alt=""></td>
                                    <td style="background:#F7F7F7;" class="nombreAsignado"><?php echo $value->nombres . " " . $value->apellidos ?></td>
                                    <td style="background:#F7F7F7;"><?php echo $value->email ?></td>
                                    <td style="background:#F7F7F7;" class="text-center">
                                    <?php if ($usuario->cargo == "Supervisor"): ?>
                                        <input class="form-check-input" type="checkbox" name="asignados[]" value="<?php echo $value->id ?>" <?php if (array_search($value->id, $asignados) !== false) {
                                            echo "checked";
                                        }  ?>>
                                        <?php endif ?>
                                    </td>
                ...
                <?php if ($usuario->cargo == "Supervisor"): ?>
                    <button type="submit" class="boton fondoAzul1 text-white"><i class="fas fa-link"></i> Asignar</button>
                <?php endif ?>
                </div>                                                                                                                           
...
function crearCirculo(circulo) {
    ...
    if(circulo.x && circulo.y){

        circulo.y = circulo.y - 15;
        let miCirculo = $("<div li='"+circulo.ide+"' num='" + circulo.num + "' id='" + circulo.id + "' xi='" + circulo.xi + "' yi='" + circulo.yi + "'  class='miCirculo miPilaComun' style='top:" + circulo.y + "px;left:" + circulo.x + "px;' x='" + circulo.x + "' y='" + circulo.y + "' pilasavanzado='" + circulo.pilasavanzado + "'>" + text_num + "</div>");
        ...
        if (empty) {
            $("#" + circulo.id).html("<img src='static/images/circulo.png' style='width:100%'><span class='textCir'>"+text_num+"</span>");
        } else if (full) {
            $("#" + circulo.id).html("<img src='static/images/rectangulo.png' style='width:100%'><span class='textCir'>"+text_num+"</span>");
        } else {
            $("#" + circulo.id).html("<img src='static/images/triangulo.png' style='width:100%'><span class='textCir'>"+text_num+"</span>");
        }
        console.log("ci : "+ circulo.y);
    }else{
        let startx = circulo.startx,
            starty = circulo.starty,
            endx = circulo.endx,
            endy = circulo.endy;

      /*   let m = Math.atan((endy - starty) / (endx - startx))*180/Math.PI, */
        let m = Math.acos((endx - startx) / Math.sqrt((endx-startx)*(endx-startx) + (-endy+starty) * (-endy+starty)))*180/Math.PI,
            l = Math.sqrt((-endy + starty) * (-endy + starty) + (endx - startx) * (endx - startx));
        if(endy - starty < 0 ){
            m = -m;
        }

        
        let miLinea = $("<div li='"+circulo.li+"' id='" + circulo.id + "'  class='miPilaComun' style='top:" + circulo.starty + "px;left:" + circulo.startx + "px;width:"+l+"px;height:2px;background:black; transform:rotate("+m+"deg);position:absolute;transform-origin:top left;'></div>");
        $("#miMapa").append(miLinea);
        console.log("linea : "+ circulo.starty);
...
if(array_coordenadas.length!=0 && array_coordenadas[0].radio>0){
    ...
    for (let i = 0; i < array_coordenadas.length; i++) {
        if(array_coordenadas[i].x && array_coordenadas[i].y){
            reX = parseFloat(array_coordenadas[i].x);
            reY = parseFloat(array_coordenadas[i].y);
        }else{
            reX = parseFloat(array_coordenadas[i].startx);
            reY = parseFloat(array_coordenadas[i].starty);
            eX = parseFloat(array_coordenadas[i].endx);
            eY = parseFloat(array_coordenadas[i].endy);
        ...
        array_coordenadas[i].li = array_coordenadas[i].startx + "-" + array_coordenadas[i].starty;
    }
    ...
    for (let i = 0; i < array_coordenadas.length; i++) {
        if(array_coordenadas[i].x && array_coordenadas[i].y){
            ...
            array_coordenadas[i].starty = array_coordenadas[i].starty - miny;
            array_coordenadas[i].starty = array_coordenadas[i].starty * factor;

            array_coordenadas[i].startx = array_coordenadas[i].startx - minx;
            array_coordenadas[i].startx = array_coordenadas[i].startx * factor;

            array_coordenadas[i].endy = array_coordenadas[i].endy - miny;
            array_coordenadas[i].endy = array_coordenadas[i].endy * factor;
            array_coordenadas[i].endx = array_coordenadas[i].endx - minx;
            array_coordenadas[i].endx = array_coordenadas[i].endx * factor;
        }
    }
    let punto = 99;
    for (let i = 0; i < array_coordenadas.length; i++) {
        if(array_coordenadas[i].x && array_coordenadas[i].y){
            ...
            if (array_coordenadas[i].starty > altoy) {
                altoy = array_coordenadas[i].starty;
                punto = i;
            }
            if (array_coordenadas[i].startx > altox) {
                altox = array_coordenadas[i].startx;
            }
            if (array_coordenadas[i].endx > altox) {
                altox = array_coordenadas[i].endx;
            }
            if (array_coordenadas[i].endy > altoy) {
                altoy = array_coordenadas[i].endy;
                punto = i;
            }
        }
    } 
    altoy = altoy + 10;
    console.log('Mayor y : ' + altoy);
    console.log('Punto : ' + punto);
    console.log('min y : ' + miny);
    for (let i = 0; i < array_coordenadas.length; i++) {
        array_coordenadas[i].y = altoy - array_coordenadas[i].y;

        if(array_coordenadas[i].starty){
            array_coordenadas[i].starty = altoy - array_coordenadas[i].starty;
            array_coordenadas[i].endy = altoy - array_coordenadas[i].endy;
        }
        crearCirculo(array_coordenadas[i]);
    }
}else{
    $("#msgError").text("Proyecto errado o vacío");
...
$(".divTotal").scrollLeft($("#miMapa").width()/2) */
...
$(function() {
    ...
    $("#eliminarProyecto").click(function(){
        location.href = "admin/proyectos/eliminar/"+$("#modalEliminarProyecto").attr("elId");
    });
            ...
            success: function(response) {
                if (response.respuesta == 1) {
                   ...
                   // $("#" + id).text(response.pilas);
                    ...
                    if (empty) {
                        $("#" + id).html("<img src='static/images/circulo.png' style='width:100%'><span class='textCir'>"+textNum+"</span>");

                    } else if (full) {
                        $("#" + id).html("<img src='static/images/rectangulo.png' style='width:100%'><span class='textCir'>"+textNum+"</span>");

                    } else {
                        $("#" + id).html("<img src='static/images/triangulo.png' style='width:100%'><span class='textCir'>"+textNum+"</span>");
